...
const EnableStep2=()=>{
    const divTobeEnabled : HTMLElement | any =  document.getElementById(`${SelectedShape}-form`);
...
const EnableStep3 =(Area:number)=>{
    ...
    const divTobeDisabled : HTMLElement | any =  document.getElementById(`${SelectedShape}-form`);
...
const shapeSelected : HTMLElement | any = document.querySelector('input[name="area"]:checked');
...
const calculateArea = () => {
    ...
    if (SelectedShape === "circle") {
        const radiusTextbox :HTMLElement|any=document.getElementById('value1-circle');
        const SelShape = new Circle(radiusTextbox.value)
         Area = SelShape.CalculateArea();
    }
    else if (SelectedShape === "rectangle") {
        const lengthTextbox :HTMLElement|any=document.getElementById('value1-rectangle');
        const breadthTextbox:HTMLElement|any=document.getElementById('value2-rectangle');
        const SelShape = new Rectangle(lengthTextbox.value,breadthTextbox.value);
        Area = SelShape.CalculateArea();
    }
    else if (SelectedShape === "square") {
        const sideTextbox :HTMLElement|any=document.getElementById('value1-square');
        const SelShape = new Square(sideTextbox.value);
        Area = SelShape.CalculateArea();
    }
    else if (SelectedShape === "ellipse") {
        const XaxisTextbox :HTMLElement|any=document.getElementById('value1-ellipse');
        const YaxisTextbox :HTMLElement|any=document.getElementById('value1-ellipse');
